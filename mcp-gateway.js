...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', (message) => {
    try {
      ...
      console.error('Error writing to MCP process stdin:', error);
      ws.close();
    }
  });
...
server.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`MCP WebSocket Gateway is running on port ${PORT}`);
  console.log(`WebSocket endpoint: ws://${ipAddress}:${PORT}`);
  console.log(`HTTP endpoint: http://${ipAddress}:${PORT}`);
});
